...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	m_v3MinimumG = v3Corner[0];
	m_v3MaximumG = v3Corner[0];

	// Get new min max of global box
	for (uint i = 1; i < 8; i++)
	{
...
bool MyRigidBody::IsColliding(MyRigidBody* const other)
{
	...
	if (bColliding)
	{
		if (this->m_v3MaximumG.x < other->m_v3MinimumG.x)
			bColliding = false;
		else if (this->m_v3MinimumG.x > other->m_v3MinimumG.x)
			bColliding = false;

		if (this->m_v3MaximumG.y < other->m_v3MinimumG.y)
			bColliding = false;
		else if (this->m_v3MinimumG.y > other->m_v3MinimumG.y)
			bColliding = false;

		if (this->m_v3MaximumG.z < other->m_v3MinimumG.z)
			bColliding = false;
		else if (this->m_v3MinimumG.z > other->m_v3MinimumG.z)
			bColliding = false;
