...
void Application::InitVariables(void)
{
	...
	m_sProgrammer = "Nick Malone - nmm3037@g.rit.edu";
...
void Application::Update(void)
{
	...
	// Set models and rigidbodies
	for (uint i = 0; i < 6; ++i)
	{
		m_lModel[i]->SetModelMatrix(glm::translate(IDENTITY_M4, m_lPosition[i]) * ToMatrix4(m_lOrientations[i]));
		m_lRigidBody[i]->SetModelMatrix(glm::translate(IDENTITY_M4, m_lPosition[i]) * ToMatrix4(m_lOrientations[i]));
	}
