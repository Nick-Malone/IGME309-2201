...
void MyMesh::GenerateDiamond(uint a_uSides, float a_fHeight, float a_fWidth, vector3 a_v3Color)
{
	...
	vector3 topTip(0, a_fHeight / 2, 0);
	vector3 botTip(0, -a_fHeight / 2, 0);

	// Create the ring of vertices at the base
	for (int i = 0; i < a_uSides; i++)
	{
		vector3 temp = vector3(cos(theta) * a_fWidth, 0, sin(theta) * a_fWidth);
	...
	{
		// The base face
		//AddTri(baseCenter, coneVertex[i], coneVertex[(i + 1) % a_uSides]);
		// Faces from tip to the bottom ring of vertices
		AddTri(topTip, coneVertex[(i + 1) % a_uSides], coneVertex[i]);
		AddTri(botTip, coneVertex[i], coneVertex[(i + 1) % a_uSides]);
	}
	// -------------------------------
